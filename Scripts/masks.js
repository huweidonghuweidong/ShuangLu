...
//Added by Manulife -->
function Mask(m, t, thousand, radix) {
    this.thousandsSeparator = thousand;
    this.radixPoint = radix;
...
Mask.prototype.attachFormat = function (o) {
    $addEvent(o, "onblur", "this.value = " + this.ref + ".formatNumber(this.value);", true);
}

Mask.prototype.detachFormat = function (o) {
    $removeEvent(o, "onblur", "this.value = " + this.ref + ".formatNumber(this.value);", true);
    this.removeFormat(o);
}

//Assume number is integer
Mask.prototype.removeFormat = function (o) {
    var radixP = this.radixPoint;
    //var regex = new RegExp("\\" + radixP + "[\\d]*|[^\\d" + radixP + "-]*", "gi");
    var regex = new RegExp("[^\\d" + radixP + "-]*", "gi");
    o.value = String(o.value).replace(regex, "");
    return o;
}

Mask.prototype.unformatNumber = function (s) {
    var radixP = this.radixPoint;
    var regex = new RegExp("[^\\d" + radixP + "-]*", "gi");
    var out = String(s).replace(regex, "");
    return out;
}

// define the removeEvent(oElement, sEvent, sCmd, bAppend) function
function $removeEvent(o, _e, c, _b) {
...
Mask.prototype.formatNumber = function (s) {
    if (this.type == "number") this.value = this.setFormattedNumber(s);
    else if (this.type == "date") this.value = this.setDate(s);
    else this.value = this.setGeneric(s);
    return this.value;
}

Mask.prototype.setFormattedNumber = function (_v, _d) {
    var regex;
    var thousandsS = this.thousandsSeparator, radixP = this.radixPoint;
    regax = new RegExp("[^\\d" + radixP + "-]*", "gi");

    var v = String(_v).replace(regax, ""), m = this.mask;

    // make sure there's only one decimal point
    regax = new RegExp("\\" + radixP);
    v = v.replace(regax, "d");
    regax = new RegExp("\\" + radixP, "g");
    v = v.replace(regax, "").replace(/d/, radixP);

    //make sure the number can be recognized by Number function
    v = v.replace(radixP, ".");

    // check to see if an invalid mask operation has been entered|([0#]{1,3}\.)
    ...
    */
    ...
    var dm = (m.indexOf(radixP) > -1) ? m.split(radixP)[1] : "";
    if (dm.length == 0) {
    ...
    */
    ...
    var im = (m.indexOf(radixP) > -1) ? m.split(radixP)[0] : m;
    im = im.replace(/[^0#]+/gi, "");
    ...
    */
    ...
    regax = new RegExp("[#0]+" + "\\" + thousandsS + "[#0]{3}");
    if (regax.test(m)) {
