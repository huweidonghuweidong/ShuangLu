        ...
        return {
          ...
          link: function (scope, element, attr, ctrl) {
            if (ctrl) {
              ...
              ////////////////////////////////////////////////
              // implement ngModelOptions in angualr js 1.3
              ////////////////////////////////////////////////
              var composing = false;
              element.on('compositionstart', function () {
                composing = true;
              ...
              var isMatch = false;
              for (var i in supportedInputType) {
                if (lowercase(attr.type) === supportedInputType[i]) {
                  isMatch = true;
                  break;
                }
              }
              if (isMatch) {
                var ngModelOptions = attr.ngModelOptions || '{ updateOn: \'blur\' }';
                ngModelOptions = scope.$eval(ngModelOptions);
                //unbind default event
                var events = $._data(element.get(0), 'events');
                for (var e in events) {
                  if (e === 'keydown' || e === 'change' || e === 'input') {
                    element.off(e);
                  }
                }
                //bind the event for update model
                element.bind(ngModelOptions.updateOn, function () {
                  if (composing)
                    return;
                  var value = element.val();
                  if (toBoolean(attr.ngTrim || 'T')) {
