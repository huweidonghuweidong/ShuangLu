eglhapplication_module.controller('eGLHAppFormPoolController', ['$scope', 'eGLHAppService', '$timeout', '$http', 'APP_CONSTANT', '$window', '$filter',
    function ($scope, eGLHAppService, $timeout, $http, appConstant, $window, $filter) {
        ...
        $scope.submittedStatus = eGLHAppService.loadCommonResourceList('submitted_eapp_status');
        ...
        $scope.steps = [];
        $scope.steps = eGLHAppService.getStepDefinition();

        $scope.$watch("selectedPendingRecord", function (record) {
            if (record) {
                $scope.selectedPendingRecordProgress = eGLHAppService.loadPendingEGLHAppData(record.eAppNumber);
            }
        });
...
//                return true;
        ...
        $scope.confirmImageUploadMethod = function () {
            eGLHAppService.callImageUpload($scope.selectedSubmittedRecord.eAppNumber);
        };
        ...
        $scope.deleteRecord = function (record) {
            if (record && record.eAppNumber) {
                ...
                ShowConfirmationDialog($filter('translate')('MESSAGE_DELETE_CONFORM'), $filter('translate')('MESSAGE_YES'), $filter('translate')('MESSAGE_NO'), function () {
                    $scope.showLoading(true);
                    eGLHAppService.deleteApp(record.eAppNumber).then(function (result) {
        ...
        $scope.initPending = function () {
            ...
            $scope.pendingRecords = eGLHAppService.loadApplicationListData($scope.requestData);
        ...
        $scope.initSubmitted = function () {
            ...
            $scope.submittedRecords = eGLHAppService.loadApplicationListData($scope.requestData);
...
//                    _isMatch = _isMatch && (client['Status'] == "05" || client['Status'] == "06");
        ...
        $scope.getApplicationStatusSummary = function (statusCode, underwritingResult, initialPaymentMethod, policyStatus) {
            ...
            return eGLHAppService.loadApplicationStatusSummary(statusCode, underwritingResult, initialPaymentMethod, policyStatus);
        };

        $scope.selectSubmitted = function (record) {
            $scope.selectedSubmittedRecord = record;
            $scope.documentDefinitions = eGLHAppService.loadDocumentDefinitions(record.PlanCode);
        };

        $scope.imageUploadEndDate = "";
        $scope.$watch("selectedSubmittedRecord", function (record) {
            if (record) {
                $scope.documentList = eGLHAppService.getAllDocuments(record.eAppNumber);

                $scope.requestData = {};
                $scope.requestData.SearchType = 'submitted';
                $scope.requestData.AppNum = record.eAppNumber;
                var result = eGLHAppService.loadApplicationListData($scope.requestData);
                if (result.length > 0) {
        ...
        $scope.getInitialPremiumMethod = function (code) {
            ...
            return eGLHAppService.loadInitialPremiumMethod(code);
        };
    }
]);
