eglhapplication_module.controller('eGLHAppFormPaymentController', ['$scope', 'eGLHAppService', '$timeout', '$http', 'APP_CONSTANT', '$window', '$interval', '$filter', '$q',
    function ($scope, eGLHAppService, $timeout, $http, appConstant, $window, $interval, $filter, $q) {
		...
		$scope.Payment.AppNum = eGLHAppService.getAppNum();
		...
		if($scope.Payment.AppNum != undefined && $scope.Payment.AppNum != ''){
			...
			$scope.GLHApp = eGLHAppService.loadApplicationData($scope.Payment.AppNum);
			
			$scope.signTerrCdList = eGLHAppService.loadSignTerrCdList($scope.Payment.AppNum);
			...
			$scope.paymentDataList = eGLHAppService.loadPaymentDataList($scope.Payment.AppNum);
			//Load Other Insured List			
			$scope.OtherInsuredDataList = eGLHAppService.loadOtherInsuredDataList($scope.Payment.AppNum);
        ...
        //加载通用信息 
        $scope.bankList = eGLHAppService.loadCommonResourceList('bank_options');
        ...
        $scope.chkBankCodeMatchBankAccountNumber = function (bankCode, bankAccountNo) {
            if (bankCode && bankAccountNo) {
                if (bankAccountNo.length > 19) {
                    ...
                    if (bankCode == "002" || bankCode == "004") //20172276
        ...
        $scope.addPaymentAccount = function () {
            $scope.Payment.BankAccountInfoList.push({
                ...
                "BankCode": "",
                "OpeningBranchName": $scope.GLHApp.AgentInfo.Terr_nm + "分行",
                "OpeningBranchLocation": $scope.GLHApp.AgentInfo.Terr_nm_with_prov,
                "BankAccountNumber": "",
...
//				});				
	...
	//						"BankAccountNumberConfirm": client.BankAccountNumberConfirm					
        ...
        $scope.clearBankAccount = function (account) {
            ...
            account.BankCode = "";
            account.OpeningBranchName = $scope.GLHApp.AgentInfo.Terr_nm + "分行";  //reset as default value
            account.OpeningBranchLocation = $scope.GLHApp.AgentInfo.Terr_nm_with_prov; //reset as default value
...
//            eGLHAppService.savePaymentData($scope.Payment).then(function (result) {
...
//        };
        ...
        $scope.Submit = function () {
           var flag = true;
           var result = eGLHAppService.savePaymentData($scope.Payment);
