			      ...
			      if(item.Current){	
			      	...
			      	item.Items.forEach(function(item2, index2){
			      		if(item2.Current){
			      			...
			      			app.currentItem.texts = '';
			      			if(app.currentItem.MsgUpInfo.length > 0){
								app.currentItem.MsgUpInfo.forEach(function(msgUpInfoItem, msgUpInfoIndex){
									app.currentItem.texts +=  msgUpInfoItem.MsgTxt;
									//app.currentHtml += 
								})	
							}
...
//				});
        ...
        nodeChange:function(nodeId,nodeIndex){
        	...
        	this.dbInputData.forEach(function(NodeItem,NodeIndex){
			    ...
			    if(NodeItem.NodeId == nodeId){
			      	NodeItem.Current = true;
			      	app.stepIndex = NodeIndex;
			      	app.stepName = nodeId;
			      	app.stepIndex = Number(NodeItem.NodeIndex);
			      	app.itemLen = NodeItem.Items.length; 
			      	...
			      	NodeItem.Items.forEach(function(item, index){
			      		if(item.ItemIndex == 0){
			      			...
			      			app.stepData.ItemId = item.ItemId;	
			      		}
			    	})
			      
			    }else{
			    	NodeItem.Current = false;
			    }
			...
			if(app.stepName != 'ND_NA70'){//houjin_将ND_NA60修复为ND_NA70
...
//			}
			...
			if(app.stepName == 'ND_NA70' && itemState >= this.itemLen -1){//houjin_将ND_NA60修复为ND_NA70
		          		...
		          		if(this.stepIndex >= 1){
		          			...
		          			//var tops = sessionStorage.getItem('layer-body' + app.callInfo.RefKey01) || 0;
			   				...
			   				var scollTimer = setTimeout(function(){
			   					$('.layer-body').scrollTop($('.layer-body')[0].scrollHeight - $('.layer-body').height());
...
//			                                    $(form).submit();
        ...
        getOut:function(){    //返回
        	...
        	if(app.signTypeAfter){
                if (app.callInfo.CallTyp == "EAPP_NOTE") {
        		    window.location = './AudioVedioNote?NoteTyp=EAPP';
                } else {
        		    window.location = './AudioVedioNote?NoteTyp=POS';
                }
        	}else{
        		this.$Modal.confirm({
	                title: "提示",
...
//			});
		...
		function pdfScroll(){
			...
			if(scrollT + pdfH >=  pdfScrollH - 768){  //768
				//alert('滚动结束');
				//app.pdfRead = true;
				app.pdfRead = true;
