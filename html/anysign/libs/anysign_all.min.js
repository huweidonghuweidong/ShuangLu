...
function anysignCommentCore(inner_string,line_count,commentObjTmp,identify_callback){var base_stroke_width=(window.innerWidth>=480?15:10);var hw;var isDown=false;var isWriting=false;var minX=9999,minY=9999,maxX=0,maxY=0;var curX=0,curY=0;var points=[];var points_all=[];var sbuilder=[];var firstPointTime=0;var signTrack="";var signTrachPointCount=0;var paste_padding=10;var lastX=0,lastY=0;var comment_canvas;var tmp_ctx;var calculatedSigWidth=0;var calculatedSigHeight=0;var C_imageDataTmp;var itarget=10;var CurrentNum=0;var myImage=new Array();var timer=null;var line_width=0;var row_height=0;var rowNum=0;var imgWidth;var imgHeight;var isPoint=false;var commentInner=inner_string.replace(/(.)(?=[^$])/g,"$1,").split(",");var comment_canvas=document.getElementById("comment_canvas");var tmp_title=document.getElementById("comment_title");var tmp_signImage=document.getElementById("signImage");var comment_ok=document.getElementById("comment_ok");var comment_back=document.getElementById("comment_back");var comment_cancel=document.getElementById("comment_cancel");tmp_ctx=comment_canvas.getContext("2d");var text=document.getElementById("signTitle");if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}tmp_title.innerHTML=commentInner[0].replace(commentInner[0],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(1,commentInner.length);var scaleFactor=window.devicePixelRatio?window.devicePixelRatio:1;tmp_ctx.strokeStyle=commentObjTmp.penColor;tmp_ctx.lineWidth=base_stroke_width;tmp_ctx.lineCap="round";tmp_ctx.lineJoin="round";tmp_ctx.shadowBlur=5;comment_ok.onclick=function(){comment_signConfirm()};comment_back.onclick=function(){comment_backCanvas()};comment_cancel.onclick=function(){comment_cancelSign()};document.body.onselectstart=document.body.oncontextmenu=function(){return false};if("ontouchstart" in document.documentElement){comment_canvas.ontouchstart=function(e){if(timer!=null){clearInterval(timer)}isDown=true;isWriting=true
};comment_canvas.ontouchmove=function(e){if(isDown){var offset=JQuery_Capable.offset(e.target);if(typeof e.targetTouches!=="undefined"){curX=Math.floor(e.targetTouches[0].pageX-offset.left);curY=Math.floor(e.targetTouches[0].pageY-offset.top)}else{curX=Math.floor(e.pageX-offset.left);curY=Math.floor(e.pageY-offset.top)}var mill=e.timeStamp;if(signTrachPointCount===0&&!isNaN(mill)){firstPointTime=mill}if(curX>0&&curY<=comment_canvas.height){if(!isNaN(mill)){signTrack+=(curX+","+curY+","+base_stroke_width+","+(mill-firstPointTime)+"\n")}else{signTrack+=(curX+","+curY+","+base_stroke_width+","+0+"\n")}signTrachPointCount+=1;if(curX>maxX){maxX=curX}if(curX<minX){minX=curX}if(curY>maxY){maxY=curY}if(curY<minY){minY=curY}points.push({x:curX,y:curY});onCommentPaint();points_all.push({x:curX,y:curY});if(curX>0&&curY>0){sbuilder.push(curX,curY)}lastX=curX;lastY=curY}preventDefault(e)}};comment_canvas.ontouchend=function(e){if(isDown){var mill=e.timeStamp;if(!isNaN(mill)){signTrack+=("0,0,"+-1+","+(mill-firstPointTime)+"\n")}else{signTrack+=("0,0,"+-1+","+0+"\n")}signTrachPointCount+=1;sbuilder.push("-1","0");var canvas=document.getElementById("comment_canvas");var ctx=canvas.getContext("2d");ctx.drawImage(comment_canvas,0,0);var text=document.getElementById("signTitle");if(CurrentNum>=commentInner.length){alert("已签署完毕！");text.innerHTML="";isWriting=false;tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height)}else{if(isPoint){isPoint=false;timer=setInterval(function(){sbuilder.push("-1","-1");if(commentObjTmp.isdistinguish&&!checkText(commentInner[CurrentNum])){var callback_dis=function(errCode,isEquals){closeWindow();if(errCode==0){if(isEquals){CurrentNum++;if(CurrentNum==commentInner.length){text.innerHTML="";tmp_title.innerHTML="已签署完毕！";tmp_title.style.color="Red";clearInterval(timer)}else{if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}tmp_title.innerHTML=inner_string;tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length)
}var image=document.createElement("img");image.id="img"+CurrentNum;image.width=commentObjTmp.mass_word_width;image.height=commentObjTmp.mass_word_height;if(checkText(commentInner[CurrentNum-1])){image.src=canvas.toDataURL("image/png")}else{var width=maxX-minX;var height=maxY-minY;var cx=(maxX+minX)/2;var cy=(maxY+minY)/2;var radius;if(width<20&&height<20){radius=commentObjTmp.mass_word_width}else{if(width>height){radius=width/2}else{radius=height/2}}C_imageDataTmp=comment_canvas.getContext("2d").getImageData(cx-radius-paste_padding,cy-radius-paste_padding,(radius+paste_padding)*2,(radius+paste_padding)*2);var signData=getCommentSigData();image.src=signData}myImage[CurrentNum-1]=image;tmp_signImage.appendChild(image);tmp_signImage.scrollTop=tmp_signImage.scrollHeight;tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;sbuilder=[];if(timer!=null){clearInterval(timer)}}}else{tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;sbuilder=[];clearInterval(timer)}if(identify_callback==null){console.log("the identify callback is not definition")}else{if(errCode!="请求超时"){identify_callback(errCode)}}};showProgress("正在识别，请稍候。。。");var ocrObj=new OCRObj(commentObjTmp.ocrCapture);ocrObj.recognition(sbuilder,commentInner[CurrentNum],callback_dis);isWriting=false;clearInterval(timer)}else{CurrentNum++;sbuilder=[];if(CurrentNum==commentInner.length){text.innerHTML="";tmp_title.innerHTML="已签署完毕！";tmp_title.style.color="Red";clearInterval(timer)}else{if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}tmp_title.innerHTML=inner_string;tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length)}var image=document.createElement("img");image.id="img"+CurrentNum;image.width=commentObjTmp.mass_word_width;
image.height=commentObjTmp.mass_word_height;if(checkText(commentInner[CurrentNum-1])){image.src=canvas.toDataURL("image/png")}else{var width=maxX-minX;var height=maxY-minY;var cx=(maxX+minX)/2;var cy=(maxY+minY)/2;var radius;if(width<20&&height<20){radius=commentObjTmp.mass_word_width}else{if(width>height){radius=width/2}else{radius=height/2}}C_imageDataTmp=comment_canvas.getContext("2d").getImageData(cx-radius-paste_padding,cy-radius-paste_padding,(radius+paste_padding)*2,(radius+paste_padding)*2);var signData=getCommentSigData();image.src=signData}myImage[CurrentNum-1]=image;tmp_signImage.appendChild(image);tmp_signImage.scrollTop=tmp_signImage.scrollHeight;tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;signData=null;isWriting=false;clearInterval(timer)}},1000)}}}isDown=false;points=[];points_all=[]};comment_canvas.ontouchcancel=comment_canvas.ontouchend}else{comment_canvas.onmousedown=function(e){if(timer!=null){clearInterval(timer)}isDown=true;isWriting=true};comment_canvas.onmousemove=function(e){if(isDown){var offset=JQuery_Capable.offset(e.target);var windowScrollOffsetTop=document.body.scrollTop|document.documentElement.scrollTop;var windowScrollOffsetLeft=document.body.scrollLeft|document.documentElement.scrollLeft;if(typeof e.targetTouches!=="undefined"){curX=Math.floor(e.targetTouches[0].clientX-offset.left);curY=Math.floor(e.targetTouches[0].clientY-offset.top)}else{curX=Math.floor(e.clientX-offset.left);curY=Math.floor(e.clientY-offset.top)}var mill=e.timeStamp;if(signTrachPointCount===0&&!isNaN(mill)){firstPointTime=mill}if(curX>0){if(!isNaN(mill)){signTrack+=(curX+","+curY+","+base_stroke_width+","+(mill-firstPointTime)+"\n")}else{signTrack+=(curX+","+curY+","+base_stroke_width+","+0+"\n")}signTrachPointCount+=1;curY+=windowScrollOffsetTop;curX+=windowScrollOffsetLeft;if(curX>maxX){maxX=curX}if(curX<minX){minX=curX}if(curY>maxY){maxY=curY}if(curY<minY){minY=curY}points.push({x:curX,y:curY});onCommentPaint();
points_all.push({x:curX,y:curY});if(curX>0&&curY>0){sbuilder.push(curX,curY)}lastX=curX;lastY=curY}preventDefault(e)}};comment_canvas.onmouseup=function(e){if(isDown){isDown=false;var offset=JQuery_Capable.offset(e.target);if(typeof e.targetTouches!=="undefined"){curX=Math.floor(e.targetTouches[0].clientX-offset.left);curY=Math.floor(e.targetTouches[0].clientY-offset.top)}else{curX=Math.floor(e.clientX-offset.left);curY=Math.floor(e.clientY-offset.top)}var mill=e.timeStamp;if(!isNaN(mill)){signTrack+=("0,0,"+-1+","+(mill-firstPointTime)+"\n")}else{signTrack+=("0,0,"+-1+","+0+"\n")}signTrachPointCount+=1;sbuilder.push("-1","0");var canvas=document.getElementById("comment_canvas");var ctx=canvas.getContext("2d");ctx.drawImage(comment_canvas,0,0);var text=document.getElementById("signTitle");if(CurrentNum>=commentInner.length){alert("已签署完毕！");text.innerHTML="";tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);isWriting=false}else{if(points_all.length>10){timer=setInterval(function(){sbuilder.push("-1","-1");if(commentObjTmp.isdistinguish&&!checkText(commentInner[CurrentNum])){var callback_dis=function(errCode,isEquals){closeWindow();if(errCode==0){if(isEquals){CurrentNum++;if(CurrentNum==commentInner.length){text.innerHTML="";tmp_title.innerHTML="已签署完毕！";tmp_title.style.color="Red";clearInterval(timer)}else{if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}tmp_title.innerHTML=inner_string;tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length)}var image=document.createElement("img");image.id="img"+CurrentNum;image.width=commentObjTmp.mass_word_width;image.height=commentObjTmp.mass_word_height;if(checkText(commentInner[CurrentNum-1])){image.src=canvas.toDataURL("image/png")}else{var width=maxX-minX;var height=maxY-minY;var cx=(maxX+minX)/2;var cy=(maxY+minY)/2;
var radius;if(width<20&&height<20){radius=commentObjTmp.mass_word_width}else{if(width>height){radius=width/2}else{radius=height/2}}C_imageDataTmp=comment_canvas.getContext("2d").getImageData(cx-radius-paste_padding,cy-radius-paste_padding,(radius+paste_padding)*2,(radius+paste_padding)*2);var signData=getCommentSigData();image.src=signData}myImage[CurrentNum-1]=image;tmp_signImage.appendChild(image);tmp_signImage.scrollTop=tmp_signImage.scrollHeight;tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;sbuilder=[];if(timer!=null){clearInterval(timer)}}}else{tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;sbuilder=[];clearInterval(timer)}if(identify_callback==null){console.log("the identify callback is not definition")}else{if(errCode!="请求超时"){identify_callback(errCode)}}};showProgress("正在识别，请稍候。。。");var ocrObj=new OCRObj(commentObjTmp.ocrCapture);ocrObj.recognition(sbuilder,commentInner[CurrentNum],callback_dis);isWriting=false;clearInterval(timer)}else{CurrentNum++;sbuilder=[];if(CurrentNum==commentInner.length){text.innerHTML="";tmp_title.innerHTML="已签署完毕！";tmp_title.style.color="Red";clearInterval(timer)}else{if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}tmp_title.innerHTML=inner_string;tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length)}var image=document.createElement("img");image.id="img"+CurrentNum;image.width=commentObjTmp.mass_word_width;image.height=commentObjTmp.mass_word_height;if(checkText(commentInner[CurrentNum-1])){image.src=canvas.toDataURL("image/png")}else{var width=maxX-minX;var height=maxY-minY;var cx=(maxX+minX)/2;var cy=(maxY+minY)/2;var radius;if(width<20&&height<20){radius=commentObjTmp.mass_word_width}else{if(width>height){radius=width/2
}else{radius=height/2}}C_imageDataTmp=comment_canvas.getContext("2d").getImageData(cx-radius-paste_padding,cy-radius-paste_padding,(radius+paste_padding)*2,(radius+paste_padding)*2);var signData=getCommentSigData();image.src=signData}myImage[CurrentNum-1]=image;tmp_signImage.appendChild(image);tmp_signImage.scrollTop=tmp_signImage.scrollHeight;tmp_ctx.clearRect(0,0,comment_canvas.width,comment_canvas.height);minX=9999,minY=9999,maxX=0,maxY=0;signData=null;isWriting=false;clearInterval(timer)}},1000)}}isDown=false;points=[];points_all=[]}};comment_canvas.onmouseout=comment_canvas.onmouseup}function getCommentSigData(){if(C_imageDataTmp){var tmp_canvas=document.createElement("canvas");var ctx=tmp_canvas.getContext("2d");var scaleFactor=window.devicePixelRatio?window.devicePixelRatio:1;tmp_canvas.width=C_imageDataTmp.width/scaleFactor;tmp_canvas.height=C_imageDataTmp.height/scaleFactor;var scaleFactor2=1;if(commentObjTmp.mass_word_width/commentObjTmp.mass_word_height<=tmp_canvas.width/tmp_canvas.height){if(commentObjTmp.mass_word_width<=tmp_canvas.width){scaleFactor2=commentObjTmp.mass_word_width/tmp_canvas.width}}else{if(commentObjTmp.singleHeight<=tmp_canvas.height){scaleFactor2=commentObjTmp.mass_word_height/tmp_canvas.height}}tmp_canvas.width*=scaleFactor2;tmp_canvas.height*=scaleFactor2;calculatedSigWidth=tmp_canvas.width;calculatedSigHeight=tmp_canvas.height;ctx.scale(1/scaleFactor*scaleFactor2,1/scaleFactor*scaleFactor2);var data_canvas=document.createElement("canvas");data_canvas.width=C_imageDataTmp.width;data_canvas.height=C_imageDataTmp.height;data_canvas.getContext("2d").putImageData(C_imageDataTmp,0,0);ctx.drawImage(data_canvas,0,0);var compressSignData=tmp_canvas.toDataURL();return compressSignData}return null}var onCommentPaint=function(){if(points.length<3){var b=points[0];tmp_ctx.beginPath();tmp_ctx.strokeStyle=commentObjTmp.penColor;tmp_ctx.fillStyle=commentObjTmp.penColor;tmp_ctx.arc(b.x,b.y,tmp_ctx.lineWidth/2,0,Math.PI*2,!0);tmp_ctx.fill();tmp_ctx.closePath();
isPoint=true;return}tmp_ctx.clearRect(0,0,tmp_ctx.width,tmp_ctx.height);tmp_ctx.beginPath();tmp_ctx.strokeStyle=commentObjTmp.penColor;tmp_ctx.fillStyle=commentObjTmp.penColor;tmp_ctx.moveTo(points[0].x,points[0].y);for(var i=1;i<points.length-2;i++){var c=(points[i].x+points[i+1].x)/2;var d=(points[i].y+points[i+1].y)/2;tmp_ctx.quadraticCurveTo(points[i].x,points[i].y,c,d)}tmp_ctx.stroke();isPoint=true};function comment_backCanvas(){if(isWriting){return}if(CurrentNum<=0){alert("已移除完，请重新进行签名！");line_width=0}else{var image=document.getElementById("img"+CurrentNum);var tmp_signImage=document.getElementById("signImage");tmp_signImage.removeChild(image);CurrentNum--;rowNum--;var text=document.getElementById("signTitle");if(commentObjTmp.isShowBgText){text.innerHTML=commentInner[CurrentNum]}if(CurrentNum==0){line_width=0;rowNum=0}else{if(rowNum-1<0){rowNum=line_count;if(row_height>0){row_height--}line_width=0}else{line_width=rowNum*commentObjTmp.mass_word_width}}var tmp_title=document.getElementById("comment_title");tmp_title.innerHTML=inner_string;tmp_title.style.color="black";tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length)}}function comment_cancelSign(){document.documentElement.style.overflow="auto";if(signResCallback){signResCallback(commentObjTmp.cid,null,null,0)}clear_all()}function comment_signConfirm(){if(isWriting){return}if(CurrentNum!=commentInner.length){custom_alert("请完成签名","确认");return}draw();if(signResCallback){signResCallback(commentObjTmp.cid,base64[0].substring(22,base64[0].length),base64[0].substring(22,base64[0].length),signTrack,signTrachPointCount,imgWidth,imgHeight)}clear_all()}function clear_all(){var dlg=document.getElementById("comment_dialog");dlg.style.display="none";minX=9999,minY=9999,maxX=0,maxY=0;document.body.parentNode.style.overflow="scroll";var commentSign=document.getElementById("comment_sign");
var tmp_textView=document.getElementById("comment_title");tmp_textView.innerHTML="";tmp_textView.style.color="black";var tmp_text=document.getElementById("signTitle");tmp_text.innerHTML="";for(var i=0;i<myImage.length;i++){tmp_signImage.removeChild(myImage[i])}commentObjTmp=null}var base64=[];function draw(){var c=document.createElement("canvas"),ctx=c.getContext("2d"),len=commentObjTmp.mass_words_in_single_line;imgWidth=commentObjTmp.mass_word_width*len;imgHeight=commentObjTmp.mass_word_height*(inner_string.length/commentObjTmp.mass_words_in_single_line+1);c.width=imgWidth;c.height=imgHeight;ctx.rect(0,0,c.width,c.height);ctx.fillStyle="#fff";ctx.fill();var w=0;var h=0;var allNum=commentInner.length;function drawing(n){if(n<allNum){if(w>=commentObjTmp.mass_words_in_single_line){h=h+commentObjTmp.mass_word_height;w=0}ctx.drawImage(myImage[n],commentObjTmp.mass_word_width*w,h,commentObjTmp.mass_word_width,commentObjTmp.mass_word_height);w++;drawing(n+1)}else{base64.push(c.toDataURL("image/png",1))}}drawing(0)}function setSignResCallback(signObj,callback){if(isSignOrCom(signObj.cid)){signObjTmp=signObj;signResCallback=callback;var canvas=document.getElementById("anysignCanvas");var ctx=canvas.getContext("2d");ctx.strokeStyle=signObjTmp?signObjTmp.penColor:"black"}else{commentObjTmp=signObj;signResCallback=callback;var canvas=document.getElementById("comment_canvas");var ctx=canvas.getContext("2d");ctx.strokeStyle=commentObjTmp.penColor;comment_canvas.strokeStyle=commentObjTmp.penColor}}window.addEventListener("resize",function(){if(timer!=null){clearInterval(timer)}var text=document.getElementById("signTitle");tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length);if(commentObjTmp.isShowBgText){if(CurrentNum>=commentInner.length){text.innerHTML=""}else{text.innerHTML=commentInner[CurrentNum]
}}tmp_ctx.strokeStyle=commentObjTmp.penColor;tmp_ctx.lineWidth=base_stroke_width;tmp_ctx.lineCap="round";tmp_ctx.lineJoin="round";tmp_ctx.shadowBlur=5});window.addEventListener("orientationchange",function(){if(timer!=null){clearInterval(timer)}var text=document.getElementById("signTitle");tmp_title.innerHTML=inner_string.substring(0,CurrentNum)+commentInner[CurrentNum].replace(commentInner[CurrentNum],'<font color="'+commentObjTmp.currentEditBarTextColor+'">$&</font>')+inner_string.substring(CurrentNum+1,commentInner.length);tmp_ctx.strokeStyle=commentObjTmp.penColor;tmp_ctx.lineWidth=base_stroke_width;tmp_ctx.lineCap="round";tmp_ctx.lineJoin="round";tmp_ctx.shadowBlur=5})};
